...
printf("sizeof(i_value) = %ld (bytes)\n",sizeof(i_value));
 ...
 printf("i_pointer (found by address) = %ls\n",i_pointer);
 printf("sizeof(i_value) = %ld (bytes)\n",sizeof(i_pointer));

// *i_pointer = (*i_pointer++); // increments šeit nestrādā
/*
jo, pirmkārt, tam ir zemāka prioritāte. 
 Otrkārt, izgūta vērtība iekš () vairs nav saistīta ne ar kādu mainīgo
 kuram šī inkrementa operācija tiktu veikta
*/
 *i_pointer = *i_pointer+1; // i++; i = i + 1
 ...
 *d_pointer = *(d_pointer)+0.25; // i++; i = i + 1
